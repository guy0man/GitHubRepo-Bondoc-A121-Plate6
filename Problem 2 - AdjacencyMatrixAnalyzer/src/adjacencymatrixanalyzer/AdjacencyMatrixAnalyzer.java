...
public class AdjacencyMatrixAnalyzer {
    ...
    public static void main(String[] args) {
        ...
        for (int a = 0; a < vertices; a++){
                System.out.println("Enter row "+ (a+1) +": ");
                String[] row = scanInput.nextLine().split(",");
                vertices = row.length;
                for (int i = 0; i < row.length; i++){
                    if (Integer.parseInt(row[i]) > 0){
                        edges.add(verticeNames[a] + verticeNames[i]);
                        edgesNum.add(row[i]);
                    }
                }               
        }
        if (edges.isEmpty()) System.out.println("There are no edges");
        else System.out.println("Edges = " + edges);
        System.out.println();
